# Generated by Powerlevel10k configuration wizard on 2024-12-05 at 19:00 PST.
...
# round separators, round heads, round tails, 1 line, sparse, many icons, fluent,
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    os_icon                 # os identifier (Unicode, not Nerd Font)
    dir                     # current directory
    vcs                     # git status
    command_execution_time  # duration of the last command (left = stays visible)
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ai_tool                 # Cursor / Claude / terminal context
  ...
  # Ubuntu / plain fonts: no Nerd Font or Powerline PUA glyphs (avoids ? boxes).
  ...
  typeset -g POWERLEVEL9K_OS_ICON_CONTENT_EXPANSION=$'\U0001F34E '
  ...
  # Folder icons (emoji — work in Ubuntu Mono without Nerd Font).
  typeset -g POWERLEVEL9K_DIR_CLASSES=(
    '~'    HOME            $'\U0001F3E0 '
    '~/*'  HOME_SUBFOLDER  $'\U0001F4C1 '
    '*'    DEFAULT         $'\U0001F4C1 '
  )
  ...
  typeset -g POWERLEVEL9K_VCS_VISUAL_IDENTIFIER_EXPANSION=$'\U0001F5C3 '
  ...
  function my_git_formatter() {
    ...
    # Branch/tracking: dark on segment bg. Status markers: accent colors (still readable on green/yellow).
    local       meta='%7F'
    local      clean='%0F'
    local   modified='%3F'
    local  untracked='%1F'
    local conflicted='%1F%B'
  ...
  # Show duration of the last command if takes at least this many seconds (0 = always).
  ...
  function prompt_ai_tool() {
    if [[ -n "${CURSOR_AGENT:-}" ]]; then
      p10k segment -b 93 -f 16 -i $'\U0001F916 ' -t 'agent'
    elif [[ -n "${CLAUDECODE:-}" || -n "${CLAUDE_CODE_SSE_PORT:-}" ]]; then
      p10k segment -b 172 -f 231 -i $'\u2726 ' -t 'Claude'
    elif [[ "${TERM_PROGRAM:-}" == vscode || -n "${VSCODE_IPC_HOOK:-}" || -n "${CURSOR_TRACE_ID:-}" ]]; then
      p10k segment -b 039 -f 231 -i $'\u25B6 ' -t 'Cursor'
    fi
  }
  ...
  function instant_prompt_ai_tool() {
    prompt_ai_tool
  }

  typeset -g POWERLEVEL9K_AI_TOOL_FOREGROUND=231
  typeset -g POWERLEVEL9K_AI_TOOL_BACKGROUND=039
  ...
  # off: keep full prompt (incl. execution time) on each line in scrollback
